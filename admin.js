...
$('loginBtn').onclick = async () => {
  ...
  if (!isFirebaseConfigured()) {
    $('loginMsg').textContent = 'ยังไม่ได้ตั้งค่า Firebase ใน firebase-config.js';
...
onAuthStateChanged(auth, async user => {
  const uidConfigured =
    ADMIN_UID &&
    !ADMIN_UID.startsWith('PASTE_');

  if (user && uidConfigured && user.uid === ADMIN_UID) {
...
async function fetchAll() {
  ...
  return {
    ...
    registrations: r.docs.map(d => ({ id: d.id, ...d.data() })),
...
function scoreOf(s) {
  ...
  (s.questionIds || []).forEach((qid, i) => {
    ...
    if (
      key &&
      Number(s.selectedOriginal?.[i]) === Number(key.correct)
    ) {
      correct++;
    }
  });
...
async function renderAll() {
  try {
    ...
    const vals = submissions.map(scoreOf);

    $('mAvg').textContent = vals.length
      ? (vals.reduce((x, y) => x + y.score, 0) / vals.length).toFixed(2)
      : '0.00';
    ...
    console.error(e);
    alert('อ่านข้อมูลจาก Firebase ไม่สำเร็จ กรุณาตรวจสอบ Firestore Rules และการตั้งค่า Firebase');
  }
}
  ...
  subs
    ...
    .forEach(s => {
      ...
      tr.innerHTML = `
        ...
        <td>${esc(s.student?.className)}</td>
        <td>${esc(s.student?.department)}</td>
        <td>${esc(s.status)}</td>
        <td>${sc.correct}/50</td>
  ...
  regs
    ...
    .forEach(r => {
      ...
      tr.innerHTML = `
        ...
        <td><button class="btn danger small">ลบ/รีเซ็ต</button></td>
      `;

      tr.querySelector('button').onclick = async () => {
        if (confirm('ลบข้อมูลลงทะเบียนเพื่ออนุญาตให้เลขนักศึกษานี้เข้าสอบใหม่?')) {
...
function renderQuestions(qs) {
  ...
  qs
    ...
    .forEach((q, i) => {
      const d = document.createElement('div');
      d.className = 'admin-question';

      d.innerHTML = `
        <div class="qtitle">ข้อ ${i + 1}. ${esc(q.q)}</div>
        <ol type="A">
...
function editQ(id) {
  const q = questionRows.find(x => x.id === id);
  const key = answerMap.get(id) || {};

  editId = id;

  $('qText').value = q.q;
  $('qLevel').value = q.level || 'basic';
  $('qCorrect').value = String(key.correct ?? 0);

  (q.options || []).forEach((o, j) => {
    if ($(`o${j}`)) $(`o${j}`).value = o;
  });

  $('qExplain').value = key.explain || '';
  $('saveQuestion').textContent = 'บันทึกการแก้ไข';
  $('cancelEdit').classList.remove('hidden');

  scrollTo({
    top: $('tab-questions').offsetTop - 20,
    behavior: 'smooth'
  });
}
...
function clearQForm() {
  ...
  ['qText', 'o0', 'o1', 'o2', 'o3', 'qExplain'].forEach(id => {
    $(id).value = '';
  });

  $('qLevel').value = 'basic';
  $('qCorrect').value = '0';
  $('saveQuestion').textContent = 'เพิ่มข้อสอบ';
...
$('saveQuestion').onclick = async () => {
  const q = {
    level: $('qLevel').value,
    q: $('qText').value.trim(),
    options: [0, 1, 2, 3].map(i => $(`o${i}`).value.trim()),
    active: true,
  ...
  const key = {
    ...
    explain: $('qExplain').value.trim(),
    updatedAt: serverTimestamp()
  };

  if (!q.q || q.options.some(x => !x)) {
    $('qMsg').textContent = 'กรุณากรอกคำถามและตัวเลือกทั้ง 4 ข้อให้ครบ';
  ...
  b.set(doc(db, 'questions', id), q, { merge: true });
  b.set(doc(db, 'answerKeys', id), key, { merge: true });
...
$('resetQuestions').textContent = 'นำเข้าชุดข้อสอบ JSON';
$('resetQuestions').onclick = () => document.getElementById('seedFile').click();

const seedInput = document.createElement('input');
seedInput.type = 'file';
seedInput.accept = 'application/json';
seedInput.id = 'seedFile';
...
seedInput.onchange = async e => {
  ...
  try {
    ...
    if (!Array.isArray(arr) || arr.length < 50) {
      throw new Error('ไฟล์ต้องเป็น array และมีอย่างน้อย 50 ข้อ');
    }

    if (!confirm(`นำเข้าข้อสอบ ${arr.length} ข้อ และแทนที่คลังข้อสอบปัจจุบันทั้งหมด?`)) {
      return;
    }

    const oldQ = await getDocs(collection(db, 'questions'));
    const oldA = await getDocs(collection(db, 'answerKeys'));

    let b = writeBatch(db);
    let n = 0;

    for (const d of [...oldQ.docs, ...oldA.docs]) {
      b.delete(d.ref);
      n++;
    ...
    for (let i = 0; i < arr.length; i++) {
      const x = arr[i];
      const id = x.id || `q-${String(i + 1).padStart(3, '0')}`;
...
$('clearResults').onclick = async () => {
  if (!confirm('ลบผลสอบทั้งหมด?')) return;

  const s = await getDocs(collection(db, 'submissions'));
  await deleteDocs(s.docs);
  await renderAll();
};

$('clearRegs').onclick = async () => {
  if (!confirm('ลบข้อมูลลงทะเบียนทั้งหมดและเปิดสิทธิ์เข้าสอบใหม่ทั้งหมด?')) return;
...
document.querySelectorAll('.tab').forEach(b => {
  b.onclick = () => {
    ...
    ['results', 'registrations', 'questions', 'backup'].forEach(t => {
...
$('exportBtn').onclick = async () => {
  ...
  a.download = `nangrong-exam-backup-${new Date().toISOString().slice(0, 10)}.json`;
  a.click();
...
$('importFile').onchange = async () => {
  alert(
    'สำหรับเวอร์ชัน Firebase ให้ใช้ปุ่ม “นำเข้าชุดข้อสอบ JSON” ในแท็บจัดการข้อสอบ ส่วนการคืนข้อมูลผลสอบควรทำจาก Firebase Console เพื่อป้องกันการเขียนข้อมูลผิดชุด'
  );
};
